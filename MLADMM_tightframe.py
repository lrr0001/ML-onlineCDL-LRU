...
class MultiLayerCSC_tightframe(optmz.ADMM):
    '''
    x: list
    ...
    def build_fft_layers(self,fftSz,noL):
        ...
        FFT_factor = []
        for ii in range(noL):
            FFT.append(tf.identity)
            IFFT.append(tf.identity)
            FFT_factor.append(1.)
        return FFT,IFFT,FFT_factor

    def build_dict_obj(self,fftSz,D,rho,n_components,cmplxdtype,layer):
        if layer == 0:
            return fctr.dictionary_object2D_tight_frame_init(D = tf.convert_to_tensor(D),rho=tf.cast(rho,dtype=cmplxdtype),objname='dict_layer' + str(layer))
        else:
            return fctr.dictionary_object2D_tight_frame_init(D = tf.convert_to_tensor(D),rho=tf.cast(rho,dtype=cmplxdtype),objname='dict_layer' + str(layer))
