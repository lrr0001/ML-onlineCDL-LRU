...
def test_FISTA_CSC_saved_dict(lpstz,noi,databasename,steps_per_epoch,num_of_epochs,pklfile):
    #   ******** ALGORITHM-SPECIFIC HYPERPARAMETERS ********
    #rho = 1.
    #lpstz = 100.
    #alpha_init = 1.5
    ...
    b_init = pkl.load(fid)
    fid.close()
    timesname = 'times/' + pklfile + modelname + 'lpstz' + str(lpstz) + '_iter' + str(noi) + '_times.pkl'
    ...
    CSC_Wrap = mlcscf.Wrap_ML_FISTA(lpstz,mu_init,b_init,qY,qUV,cropAndMerge,fftSz,strides,[tf.reshape(D[ii],(1,) + D[ii].shape) for ii in range(len(D))],noi,noL,cmplxdtype)
...
def test_FISTA_CSC_init(lpstz,noi,databasename,steps_per_epoch,num_of_epochs,pklfile):
    #   ******** ALGORITHM-SPECIFIC HYPERPARAMETERS ********
    #rho = 1.
    #lpstz = 100.
    #alpha_init = 1.5
    ...
    CSC_Wrap = mlcscf.Wrap_ML_FISTA(lpstz,mu_init,b_init,qY,qUV,cropAndMerge,fftSz,strides,problem_param['D'],noi,noL,cmplxdtype)
    Get_Obj = mlcscf.Get_Obj(CSC_Wrap)
