...
checkpointfilename = modelname + 'checkpoint_epoch_continue{epoch:02d}.ckpt'
...
testfile = problem_param['testfile']
...
raw_dataset = tf.data.TFRecordDataset([datapath + testfile])
...
datainput = dataset_batch.__iter__().__next__()
results = model.predict(x = datainput)
#results.append(model.evaluate(x = dataset_batch,steps = steps_per_epoch,return_dict=True))
#    results2.append(model2.evaluate(x = dataset_batch,steps = steps_per_epoch,return_dict=True))
#    results3.append(model3.evaluate(x = dataset_batch,steps = steps_per_epoch,return_dict=True))

fid = open(experimentpath + modelname + 'sampleOutput.pkl','wb')
pkl.dump((datainput,results),fid)
fid.close()
