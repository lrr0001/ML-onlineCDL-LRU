...
class ifft2d(fft2d):
    def call(self,inputs):
        return tf.signal.irfft(input_tensor=inputs,fft_length=self.fftSz)

class fft2d_inner(fft2d):
    def call(self,inputs):
        return switch_spatial_and_channelfltr(fft2d(switch_spatial_and_channelfltr(inputs)))

class ifft2d_inner(fft2d):
    def call(self,inputs):
        return switch_spatial_and_channelfltr(ifft2d(switch_spatial_and_channelfltr(inputs)))
